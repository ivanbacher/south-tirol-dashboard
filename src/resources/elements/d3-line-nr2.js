...
export class D3LineNr2 {
  ...
  attached() {
    ...
    y.domain([
      Math.min( d3.min( lineData, (d) =>  { return d.total_M; }), d3.min( lineData, (d) =>  { return d.total_F; }) ),
      Math.max( d3.max( lineData, (d) =>  { return d.total_M; }), d3.max( lineData, (d) =>  { return d.total_F; }) )
    ]);
